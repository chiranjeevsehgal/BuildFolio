...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setStats({
          totalUsers:
            response.data.stats?.totalUsers || response.data.users?.length || 0,
          activeUsers:
            response.data.stats?.activeUsers ||
            response.data.users?.filter((u) => u.isActive).length ||
            0,
          deployedPortfolios:
            response.data.stats?.deployedPortfolios ||
            response.data.users?.filter((u) => u.portfolioDeployed).length ||
            0,
          premiumUsers:
            response.data.stats?.premiumUsers ||
            response.data.users?.filter((u) => u.subscriptionType === "premium")
              .length ||
            0,
        });
  ...
  const updateUserStatus = async (userId, action) => {
    try {
      ...
      if (response.data.success) {
        setMessage({
          type: "success",
          content: `User ${action}d successfully`,
        });
  ...
  const bulkUpdateUsers = async (userIds, action) => {
    try {
      ...
      if (response.data.success) {
        setMessage({
          type: "success",
          content: `${userIds.length} users ${action}d successfully`,
        });
        setSelectedUsers([]);
        fetchUsers();
      }
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesSubscription =
      ...
      user.subscriptionType === filterSubscription;
  ...
  return (
    <div className="space-y-6">
      ...
      {message.content && (
        <div
          className={`fixed top-4 right-4 z-50 rounded-2xl p-4 flex items-center space-x-3 shadow-2xl backdrop-blur-sm max-w-sm transform transition-all duration-300 ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredUsers.map((user) => {
                    ...
                    return (
                      <div
                        key={user._id}
                        className={`bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border-2 transition-all duration-300 hover:shadow-xl cursor-pointer ${
                          isSelected
                      ...
                      >
                        <div className="p-6">
                          ...
                          <div className="flex items-center justify-between mb-4">
                            <div
                              className={`flex items-center space-x-1 px-2 py-1 rounded-lg border text-xs font-medium ${statusConfig.color}`}
                            >
                              <StatusIcon className="w-3 h-3" />
                              <span>{statusConfig.label}</span>
                            </div>
                            <div
                              className={`px-2 py-1 rounded-lg text-xs font-medium ${getSubscriptionBadge(user.subscriptionType)}`}
                            >
                          ...
                          <div className="grid grid-cols-1 mb-4 text-center">
                            <div className="bg-gray-50 rounded-lg p-3">
                              <div
                                className={`text-lg font-bold ${user.portfolioDeployed ? "text-green-600" : "text-gray-400"}`}
                              >
                        ...
                        {filteredUsers.map((user) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-6 py-4">
                                <div
                                  className={`inline-flex items-center space-x-1 px-2 py-1 rounded-lg border text-xs font-medium ${statusConfig.color}`}
                                >
                                  <StatusIcon className="w-3 h-3" />
                                  <span>{statusConfig.label}</span>
                                </div>
                              ...
                              <td className="px-6 py-4">
                                <div
                                  className={`inline-flex px-2 py-1 rounded-lg text-xs font-medium ${getSubscriptionBadge(user.subscriptionType)}`}
                                >
                              ...
                              <td className="px-6 py-4">
                                <div
                                  className={`text-sm font-medium ${user.portfolioDeployed ? "text-green-600" : "text-gray-500"}`}
                                >
                                  {user.portfolioDeployed ? "Live" : "Draft"}
                                </div>
                              </td>

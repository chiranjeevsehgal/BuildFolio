...
import { User, X, Edit3, Ban, Send, ExternalLink, Mail, Calendar, MapPin, Clock, Globe, Briefcase, UserCheck } from 'lucide-react';

const UserModal = ({ user, isOpen, onClose, getStatusConfig, getSubscriptionBadge  }) => {
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-t-3xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">{user.fullName || 'User Details'}</h2>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={`flex items-center space-x-2 px-3 py-1 rounded-xl border ${statusConfig.color}`}>
                  <StatusIcon className="w-4 h-4" />
                  <span className="font-medium">{statusConfig.label}</span>
                </div>
                <div className={`px-3 py-1 rounded-xl font-medium text-sm ${getSubscriptionBadge(user.subscriptionType)}`}>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-1 flex items-center space-x-2">
                    ...
                    <span className="text-gray-900">
                      {user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString() : 'NA'}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center space-x-2">
                    ...
                    <span className="text-gray-900">{user.selectedTemplate || 'None'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-4">
              <h3 className="font-semibold text-gray-800 mb-3">Portfolio Status</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${user.portfolioDeployed ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                  <span className="text-sm text-gray-600">
                    {user.portfolioDeployed ? 'Deployed' : 'Not Deployed'}
                  </span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${user.isProfileCompleted ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                  <span className="text-sm text-gray-600">

...
const ProfessionalSummarySection = ({
  ...
  // Enhanced display component for read-only sections with responsive design
  const DisplayField = ({ label, value, icon: Icon, className = "" }) => (
    <div
      className={`group p-4 sm:p-5 bg-white rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-300 ${className}`}
    >
  ...
  // Skills display component with responsive design
  const SkillsDisplay = () => (
    <div className="group p-4 sm:p-5 bg-white rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-300">
      <div className="flex items-start space-x-3">
        <Award className="w-4 h-4 text-gray-400 mt-1 flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <span className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">
            Skills
          </span>
          {profileData.professional.skills.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {profileData.professional.skills.map((skill, index) => (
                <span
                  key={index}
                  className="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200 break-all"
                >
                  {skill}
                </span>
              ))}
  ...
  const SaveButtonTooltip = ({
    ...
    <div className="relative inline-block">
      ...
      {show && (
        <div
          className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-sm text-white bg-gray-800 rounded-lg shadow-lg whitespace-nowrap z-50 sm:whitespace-normal sm:max-w-xs sm:text-center"
          style={{
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
          <button
            ...
            className={`flex items-center justify-center space-x-2 px-4 py-2.5 cursor-pointer rounded-lg transition-all duration-200 text-sm sm:text-base font-medium w-full sm:w-auto sm:min-w-[120px] ${
              editingSections.professional
          ...
          {editingSections.professional && (
            ...
            >
              <button
                ...
                className={`flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg transition-all duration-200 text-sm sm:text-base font-medium w-full sm:w-auto sm:min-w-[120px] ${saveButtonState.className}`}
              >
                  ...
                  {saveButtonState.text.includes("Saving")
                    ...
                    : saveButtonState.text.includes("Saved")
                      ...
                      : saveButtonState.text.includes("Save")
                        ? "Save"
                        : saveButtonState.text.includes("Fix")
                          ? "Fix"
                          : "Save"}
                </span>
              </button>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleInputChange("professional", "title", e.target.value)
              }
              className={`w-full px-4 py-3 border rounded-lg placeholder:text-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${
                hasFieldError("title")
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleInputChange("professional", "summary", e.target.value)
              }
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 placeholder:text-gray-500 focus:ring-blue-500 focus:border-transparent resize-none transition-all duration-200 text-sm sm:text-base ${
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {profileData.professional.skills.map((skill, index) => (
                <span
                  key={index}
                  className="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-blue-50 text-blue-700 border border-blue-200 transition-all duration-200 hover:bg-blue-100 break-all"
                >
                  ...
                  <button
                    ...
                    className="ml-1 sm:ml-2 text-blue-600 cursor-pointer hover:text-red-600 transition-colors hover:bg-red-100 rounded-full p-0.5 flex-shrink-0"
                  >
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 sm:py-2 border border-slate-300 rounded-lg focus:ring-2 placeholder:text-gray-500 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base"
                placeholder="Add a skill"
              />
              <button
                onClick={addSkill}
                className="px-4 py-2 cursor-pointer bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-200 hover:shadow-md transform hover:scale-105 self-stretch sm:self-auto"
              >

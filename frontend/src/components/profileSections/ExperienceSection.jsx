...
const ExperienceSection = ({
  ...
  const getFieldError = (fieldName) => {
    return validationErrors[fieldName] &&
      (touchedFields[fieldName] || saveAttempted)
  ...
  const getCurrentData = useCallback(
    () => createExperienceCopy(profileData.experience),
  ...
  const hasDateRangeError = (index) => {
    return (
      !!validationErrors[`experience_${index}_dateRange`] &&
      (touchedFields[`experience_${index}_startDate`] ||
        touchedFields[`experience_${index}_endDate`] ||
        saveAttempted)
    );
  ...
  // Enhanced read-only experience card with responsive design
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:space-x-4 space-y-3 sm:space-y-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-2 space-y-2 sm:space-y-0">
            ...
            {exp.current && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200 self-start">
                Current
              </span>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  {hasFieldError(`experience_${index}_location`) && (
                    <p className="mt-1 text-sm text-red-600 flex items-center space-x-1 animate-slide-down">
                      <Info className="w-3 h-3 flex-shrink-0" />
                      <span>
                        {getFieldError(`experience_${index}_location`)}
                      </span>
                    </p>
                  )}
                </div>
                <div>
                  ...
                  {hasFieldError(`experience_${index}_startDate`) && (
                    <p className="mt-1 text-sm text-red-600 flex items-center space-x-1 animate-slide-down">
                      <Info className="w-3 h-3 flex-shrink-0" />
                      <span>
                        {getFieldError(`experience_${index}_startDate`)}
                      </span>
                    </p>
                  )}
                </div>
                <div>
                  ...
                  <CustomDateInput
                    ...
                    placeholder={
                      exp.current ? "Currently working here" : "Select end date"
                    }

...
const PersonalInfoSection = ({
  ...
  const getCurrentData = useCallback(() => {
    ...
    socialLinkConfigs.forEach((config) => {
      socialLinks[config.key] =
        profileData.personalInfo.socialLinks[config.key] || "";
    });
    ...
    if (editingSections.personalInfo && hasChanges) {
      ...
      if (originalData) {
        ...
        socialLinkConfigs.forEach((config) => {
          updateSocialLinks(
            config.key,
            originalData.socialLinks[config.key] || "",
          );
        });
      }
    }
  ...
  const handleSave = async (e) => {
    ...
    const fieldsToValidate = [
      "phone",
      "location",
      ...socialLinkConfigs.map((config) => config.key),
    ];
          ...
          {value ? (
            isLink ? (
              <button
                ...
                className="text-blue-600 cursor-pointer hover:text-blue-700 transition-colors text-sm font-medium underline decoration-1 underline-offset-2 hover:decoration-2 break-all"
              >
                {value}
              </button>
            ) : (
    ...
    <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8 border border-slate-200 w-full max-w-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-6 mb-8">
        <div className="relative group self-center sm:self-start">
          <div className="w-20 h-20 sm:w-24 sm:h-24 bg-slate-200 rounded-full flex items-center justify-center overflow-hidden ring-2 ring-slate-200 transition-all duration-200 group-hover:ring-blue-300">
            {profilePhoto ? (
              <img
              ...
              <Camera className="w-6 h-6 sm:w-8 sm:h-8 text-slate-400 transition-colors group-hover:text-blue-500" />
            )}
            {uploadingPhoto && (
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mb-4 sm:mb-6">
              Add your online presence to help others connect with you
            </p>

...
const PasswordChangeModal = ({ showPasswordModal, setShowPasswordModal, setLoading, loading, showMessage: showMessageParent }) => {
    ...
    const confirmPasswordChange = async () => {
        ...
        if (passwordData.newPassword.length < 8) {
            showLocalMessage('warning', 'New password must be at least 8 characters long.');
            return;
        }
        ...
        if (passwordData.currentPassword === passwordData.newPassword) {
            showLocalMessage('warning', 'New password must be different from your current password.');
            return;
        }
        ...
        try {
            const response = await axios.put('/profiles/change-password', {
            ...
            if (error.response?.status === 400) {
                if (error.response.data.errors && Array.isArray(error.response.data.errors)) {
                    ...
                    errorMessage = error.response.data.message || 'Invalid password data provided.';
                }

        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex items-center justify-center">
          <div className="text-center">
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl">
              ...
              <h3 className="text-xl font-semibold text-slate-800 mb-2">Loading Job Tracker</h3>
              <p className="text-slate-600">Organizing your job applications...</p>
            </div>
          </div>
        </div>
      </>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 -mt-4 sm:-mt-6 lg:-mt-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
            {columns.map((column) => {
              ...
              return (
                <div key={column.id} className={`${getColumnHeaderColor(column.color)} rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 text-center shadow-lg`}>
                  <IconComponent className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 mx-auto mb-2 sm:mb-3" />
                  <div className="text-xl sm:text-2xl lg:text-3xl font-bold mb-1">{count}</div>
                  <div className="text-xs sm:text-sm lg:text-base opacity-90">{column.title}</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="mb-6 sm:mb-8 lg:mb-10">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-5">
              {columns.map((column) => {
                ...
                const minHeight = hasJobs ? 'min-h-[400px]' : 'min-h-[250px]';
                // Set max height based on screen size and make scrollable after certain number of items
                const maxHeight = 'max-h-[60vh]'; // Use viewport height for better responsiveness

                return (
                  <div
                    key={column.id}
                    className={`bg-gradient-to-br ${getColumnColor(column.color)} backdrop-blur-sm rounded-xl sm:rounded-2xl border-2 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col ${minHeight} w-full`}
                    onDragOver={handleDragOver}
                    onDrop={(e) => handleDrop(e, column.id)}
                  >
                    {/* Enhanced Column Header - No padding */}
                    <div className={`${getColumnHeaderColor(column.color)} px-3 sm:px-4 py-3 sm:py-4 shadow-sm flex-shrink-0`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <IconComponent className="w-4 h-4 sm:w-5 sm:h-5" />
                          <h3 className="font-bold text-sm sm:text-base lg:text-lg truncate">{column.title}</h3>
                        </div>
                        <div className="bg-white/20 backdrop-blur-sm px-2 sm:px-3 py-1 rounded-full flex-shrink-0">
                          <span className="text-xs sm:text-sm font-bold">{columnJobs.length}</span>
                        </div>
                      </div>
                    </div>

                    {/* Job Cards Container - Optimized for scrolling */}
                    <div className={`flex-1 ${hasJobs ? 'p-3 sm:p-4' : 'p-2'} ${maxHeight} overflow-y-auto custom-scrollbar`}>
                      {hasJobs ? (
                        <div className="space-y-3 sm:space-y-4">
                          {columnJobs.map((job) => (
                            <div
                              ...
                              className={`bg-white/95 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4 shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer border-l-4 ${getPriorityColor(job.priority)} group hover:scale-[1.02] hover:-translate-y-1`}
                            >
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-slate-400 py-6 sm:py-8">
                          ...
                          <p className="text-xs sm:text-sm font-medium text-center px-2">No jobs in {column.title.toLowerCase()}</p>
                          <p className="text-xs mt-1 text-center opacity-75 hidden sm:block px-2">Drag jobs here</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between space-y-6 sm:space-y-8 lg:space-y-0">
                ...
                <div className="text-center lg:text-left lg:flex-1">
                  <div className="flex items-center justify-center lg:justify-start mb-4">
                    ...
                    <div>
                      <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1 sm:mb-2">AI-Powered Insights</h3>
                      <p className="text-purple-100 text-sm sm:text-base lg:text-lg">Supercharge your job search with intelligent recommendations</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 lg:flex-1 lg:max-w-3xl w-full">
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 text-center hover:bg-white/20 transition-all duration-300 cursor-pointer group">
                    ...
                    <h4 className="font-semibold text-white mb-1 sm:mb-2 text-sm sm:text-base">Resume Tips</h4>
                    <p className="text-purple-100 text-xs sm:text-sm">AI-optimized suggestions</p>
                  </div>

                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 text-center hover:bg-white/20 transition-all duration-300 cursor-pointer group">
                    <Zap className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-200 mx-auto mb-2 sm:mb-3 group-hover:scale-110 transition-transform" />
                    <h4 className="font-semibold text-white mb-1 sm:mb-2 text-sm sm:text-base">Job Matching</h4>
                    <p className="text-purple-100 text-xs sm:text-sm">Smart skill analysis</p>
                  </div>

                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 text-center hover:bg-white/20 transition-all duration-300 cursor-pointer group">
                    <Target className="w-6 h-6 sm:w-8 sm:h-8 text-green-200 mx-auto mb-2 sm:mb-3 group-hover:scale-110 transition-transform" />
                    <h4 className="font-semibold text-white mb-1 sm:mb-2 text-sm sm:text-base">Success Prediction</h4>
                    <p className="text-purple-100 text-xs sm:text-sm">Probability scoring</p>
                  </div>
                </div>
              </div>
            </div>

...
export const VALIDATION_RULES = {
  personal: {
    phone: {
      ...
      pattern: /^(\+91|0)?\s*\d{10}$/,
      message:
        "Enter a valid phone number (10 digits, optionally prefixed with +91 or 0)",
    },
    location: {
      required: false,
      minLength: 2,
  ...
  professional: {
    ...
    startDate: {
      required: true,
      message: "Please provide a start date for your professional experience",
    },
    endDate: {
      required: true,
      message: "Please provide a end date for your professional experience",
    },
  },
  experience: {
    title: {
...
export const validateField = (value, rules, item = null) => {
  ...
  if (
    ...
    typeof rules.conditionalRequired === "function"
  ) {
  ...
  if (hasValue || hasArrayValue) {
    ...
    if (isArray) {
      if (rules.minCount && value.length < rules.minCount) {
        errors.push(
          rules.message || `At least ${rules.minCount} items required`,
        );
      }
...
export const validatePersonalInfo = (personalInfo) => {
  ...
  if (personalInfo.socialLinks?.linkedin) {
    const linkedinErrors = validateField(
      personalInfo.socialLinks.linkedin,
      rules.linkedin,
    );
  ...
  if (personalInfo.socialLinks?.github) {
    const githubErrors = validateField(
      personalInfo.socialLinks.github,
      rules.github,
    );
...
export const validateProfessionalInfo = (professionalInfo) => {
  ...
  if (professionalInfo.summary) {
    const summaryErrors = validateField(
      professionalInfo.summary,
      rules.summary,
    );
  ...
  if (professionalInfo.startDate) {
    const skillsErrors = validateField(
  ...
  experience.forEach((exp, index) => {
    ...
    if (hasExpData) {
      ...
      if (dateRangeErrors.length > 0) {
        errors[`experience_${index}_dateRange`] = dateRangeErrors;
      }
    }
  });
...
export const validateCompleteProfile = (profileData) => {
  ...
  const professionalValidation = validateProfessionalInfo(
...
export const checkSectionValidity = (sectionValidation) => {
  return {
    personal: sectionValidation.personal.hasData
      ? sectionValidation.personal.isValid
      : true,
    professional: sectionValidation.professional.hasData
      ? sectionValidation.professional.isValid
      : true,
    experience: sectionValidation.experience.hasData
      ? sectionValidation.experience.isValid
      : true,
    education: sectionValidation.education.hasData
      ? sectionValidation.education.isValid
      : true,
    projects: sectionValidation.projects.hasData
      ? sectionValidation.projects.isValid
      : true,
  };
};

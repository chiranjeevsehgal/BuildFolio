...
class LinkedInService {
  ...
  async getProfile(accessToken) {
    try {
      ...
      const positionsResponse = await axios.get(`${this.baseURL}/positions`, {
        ...
        params: {
          projection: '(elements*(id,title,company,location,startDate,endDate,current,summary))'
        }
      });
      ...
      const educationResponse = await axios.get(`${this.baseURL}/educations`, {
        ...
        params: {
          projection: '(elements*(id,schoolName,degree,fieldOfStudy,startDate,endDate,activities,notes))'
        }
      });
      ...
      return {
        ...
        experience: this.processExperience(positionsResponse.data?.elements || []),
        education: this.processEducation(educationResponse.data?.elements || []),
  ...
  processExperience(positions) {
    return positions.map(position => ({
      ...
      endDate: position.endDate ? this.formatLinkedInDate(position.endDate) : null,
  ...
  processEducation(educations) {
    return educations.map(education => ({
      ...
      endDate: education.endDate ? this.formatLinkedInDate(education.endDate) : null,

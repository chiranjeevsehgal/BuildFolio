    ...
    try {
      ...
      if (!validAnalysisTypes.includes(analysisType)) {
        return res.status(400).json({
          success: false,
          message:
            "Invalid analysis type. Must be one of: resumeTips, jobMatching, successPrediction",
        });
      }
      ...
      try {
        if (file.mimetype === "application/pdf") {
          resumeText = await AIInsightsController.extractTextFromPDF(
            file.buffer,
          );
        } else if (
          ...
          resumeText = await AIInsightsController.extractTextFromWord(
            file.buffer,
          );
...
${AI_PROMPTS[analysisType]}`;
        ...
        console.log(
          `AI Analysis successful with API key index ${(geminiKeyManager.getCurrentIndex() - 1 + geminiKeyManager.apiKeys.length) % geminiKeyManager.apiKeys.length}`,
        );
        return parsedData;
      } catch (error) {
        console.error(
          `AI Analysis attempt ${attempt + 1} failed:`,
          error.message,
        );
        lastError = error;
    ...
    throw new Error(
      `Failed to analyze with AI after trying all ${maxRetries} API keys. Last error: ${lastError?.message}`,
    );
  };
